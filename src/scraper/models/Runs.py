...
class Runs:
    ...
    def add(self,startdatetime,enddatetime,linkcount,scraperid):
        try:
            con = self.__connect()
            with con:
                cur = con.cursor()
                cur.execute("INSERT INTO runs(startdatetime,enddatetime,linkcount,scraperid) VALUES(%s,%s,%s,%s)",
                            (self.__sanitize(startdatetime),self.__sanitize(enddatetime),self.__sanitize(linkcount),self.__sanitize(scraperid))
                           )
                cur.close()
                newid = cur.lastrowid
            con.close()
    ...
    def update(self,runid,startdatetime,enddatetime,linkcount,scraperid):
        try:
            con = self.__connect()
            with con:
                cur = con.cursor()
                cur.execute("UPDATE runs SET startdatetime = %s,enddatetime = %s,linkcount = %s,scraperid = %s WHERE runid = %s",
                            (self.__sanitize(startdatetime),self.__sanitize(enddatetime),self.__sanitize(linkcount),self.__sanitize(scraperid),self.__sanitize(runid))
                           )
                cur.close()
            con.close()
        except Exception, e:
