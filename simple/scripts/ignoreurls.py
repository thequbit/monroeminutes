...
class ignoreurls:
    ...
    def add(self,url,ignoredt,scrapeurlid):
        with self.__con:
            ...
            cur.execute("INSERT INTO ignoreurls(url,ignoredt,scrapeurlid) VALUES(%s,%s,%s)",(self.__sanitize(url),self.__sanitize(ignoredt),self.__sanitize(scrapeurlid)))
            cur.close()
            newid = cur.lastrowid
        return newid
    ...
    def update(self,ignoreurlid,url,ignoredt,scrapeurlid):
        with self.__con:
            cur = self.__con.cursor()
            cur.execute("UPDATE ignoreurls SET url = %s,ignoredt = %s,scrapeurlid = %s WHERE ignoreurlid = %s",(self.__sanitize(url),self.__sanitize(ignoredt),self.__sanitize(scrapeurlid),self.__sanitize(ignoreurlid)))
            cur.close()
